// generated from rosidl_typesupport_c/resource/idl__type_support.cpp.em
// with input from geographic_msgs:srv/UpdateGeographicMap.idl
// generated code does not contain a copyright notice

#include "cstddef"
#include "rosidl_runtime_c/message_type_support_struct.h"
#include "geographic_msgs/srv/detail/update_geographic_map__struct.h"
#include "geographic_msgs/srv/detail/update_geographic_map__type_support.h"
#include "geographic_msgs/srv/detail/update_geographic_map__functions.h"
#include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/message_type_support_dispatch.h"
#include "rosidl_typesupport_c/type_support_map.h"
#include "rosidl_typesupport_c/visibility_control.h"
#include "rosidl_typesupport_interface/macros.h"

namespace geographic_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _UpdateGeographicMap_Request_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _UpdateGeographicMap_Request_type_support_ids_t;

static const _UpdateGeographicMap_Request_type_support_ids_t _UpdateGeographicMap_Request_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _UpdateGeographicMap_Request_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _UpdateGeographicMap_Request_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _UpdateGeographicMap_Request_type_support_symbol_names_t _UpdateGeographicMap_Request_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, geographic_msgs, srv, UpdateGeographicMap_Request)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, geographic_msgs, srv, UpdateGeographicMap_Request)),
  }
};

typedef struct _UpdateGeographicMap_Request_type_support_data_t
{
  void * data[2];
} _UpdateGeographicMap_Request_type_support_data_t;

static _UpdateGeographicMap_Request_type_support_data_t _UpdateGeographicMap_Request_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _UpdateGeographicMap_Request_message_typesupport_map = {
  2,
  "geographic_msgs",
  &_UpdateGeographicMap_Request_message_typesupport_ids.typesupport_identifier[0],
  &_UpdateGeographicMap_Request_message_typesupport_symbol_names.symbol_name[0],
  &_UpdateGeographicMap_Request_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t UpdateGeographicMap_Request_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_UpdateGeographicMap_Request_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &geographic_msgs__srv__UpdateGeographicMap_Request__get_type_hash,
  &geographic_msgs__srv__UpdateGeographicMap_Request__get_type_description,
  &geographic_msgs__srv__UpdateGeographicMap_Request__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace geographic_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, geographic_msgs, srv, UpdateGeographicMap_Request)() {
  return &::geographic_msgs::srv::rosidl_typesupport_c::UpdateGeographicMap_Request_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "geographic_msgs/srv/detail/update_geographic_map__struct.h"
// already included above
// #include "geographic_msgs/srv/detail/update_geographic_map__type_support.h"
// already included above
// #include "geographic_msgs/srv/detail/update_geographic_map__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace geographic_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _UpdateGeographicMap_Response_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _UpdateGeographicMap_Response_type_support_ids_t;

static const _UpdateGeographicMap_Response_type_support_ids_t _UpdateGeographicMap_Response_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _UpdateGeographicMap_Response_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _UpdateGeographicMap_Response_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _UpdateGeographicMap_Response_type_support_symbol_names_t _UpdateGeographicMap_Response_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, geographic_msgs, srv, UpdateGeographicMap_Response)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, geographic_msgs, srv, UpdateGeographicMap_Response)),
  }
};

typedef struct _UpdateGeographicMap_Response_type_support_data_t
{
  void * data[2];
} _UpdateGeographicMap_Response_type_support_data_t;

static _UpdateGeographicMap_Response_type_support_data_t _UpdateGeographicMap_Response_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _UpdateGeographicMap_Response_message_typesupport_map = {
  2,
  "geographic_msgs",
  &_UpdateGeographicMap_Response_message_typesupport_ids.typesupport_identifier[0],
  &_UpdateGeographicMap_Response_message_typesupport_symbol_names.symbol_name[0],
  &_UpdateGeographicMap_Response_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t UpdateGeographicMap_Response_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_UpdateGeographicMap_Response_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &geographic_msgs__srv__UpdateGeographicMap_Response__get_type_hash,
  &geographic_msgs__srv__UpdateGeographicMap_Response__get_type_description,
  &geographic_msgs__srv__UpdateGeographicMap_Response__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace geographic_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, geographic_msgs, srv, UpdateGeographicMap_Response)() {
  return &::geographic_msgs::srv::rosidl_typesupport_c::UpdateGeographicMap_Response_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "geographic_msgs/srv/detail/update_geographic_map__struct.h"
// already included above
// #include "geographic_msgs/srv/detail/update_geographic_map__type_support.h"
// already included above
// #include "geographic_msgs/srv/detail/update_geographic_map__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace geographic_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _UpdateGeographicMap_Event_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _UpdateGeographicMap_Event_type_support_ids_t;

static const _UpdateGeographicMap_Event_type_support_ids_t _UpdateGeographicMap_Event_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _UpdateGeographicMap_Event_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _UpdateGeographicMap_Event_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _UpdateGeographicMap_Event_type_support_symbol_names_t _UpdateGeographicMap_Event_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, geographic_msgs, srv, UpdateGeographicMap_Event)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, geographic_msgs, srv, UpdateGeographicMap_Event)),
  }
};

typedef struct _UpdateGeographicMap_Event_type_support_data_t
{
  void * data[2];
} _UpdateGeographicMap_Event_type_support_data_t;

static _UpdateGeographicMap_Event_type_support_data_t _UpdateGeographicMap_Event_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _UpdateGeographicMap_Event_message_typesupport_map = {
  2,
  "geographic_msgs",
  &_UpdateGeographicMap_Event_message_typesupport_ids.typesupport_identifier[0],
  &_UpdateGeographicMap_Event_message_typesupport_symbol_names.symbol_name[0],
  &_UpdateGeographicMap_Event_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t UpdateGeographicMap_Event_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_UpdateGeographicMap_Event_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &geographic_msgs__srv__UpdateGeographicMap_Event__get_type_hash,
  &geographic_msgs__srv__UpdateGeographicMap_Event__get_type_description,
  &geographic_msgs__srv__UpdateGeographicMap_Event__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace geographic_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, geographic_msgs, srv, UpdateGeographicMap_Event)() {
  return &::geographic_msgs::srv::rosidl_typesupport_c::UpdateGeographicMap_Event_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
#include "rosidl_runtime_c/service_type_support_struct.h"
// already included above
// #include "geographic_msgs/srv/detail/update_geographic_map__type_support.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/service_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"
#include "service_msgs/msg/service_event_info.h"
#include "builtin_interfaces/msg/time.h"

namespace geographic_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{
typedef struct _UpdateGeographicMap_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _UpdateGeographicMap_type_support_ids_t;

static const _UpdateGeographicMap_type_support_ids_t _UpdateGeographicMap_service_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _UpdateGeographicMap_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _UpdateGeographicMap_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _UpdateGeographicMap_type_support_symbol_names_t _UpdateGeographicMap_service_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, geographic_msgs, srv, UpdateGeographicMap)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_introspection_c, geographic_msgs, srv, UpdateGeographicMap)),
  }
};

typedef struct _UpdateGeographicMap_type_support_data_t
{
  void * data[2];
} _UpdateGeographicMap_type_support_data_t;

static _UpdateGeographicMap_type_support_data_t _UpdateGeographicMap_service_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _UpdateGeographicMap_service_typesupport_map = {
  2,
  "geographic_msgs",
  &_UpdateGeographicMap_service_typesupport_ids.typesupport_identifier[0],
  &_UpdateGeographicMap_service_typesupport_symbol_names.symbol_name[0],
  &_UpdateGeographicMap_service_typesupport_data.data[0],
};

static const rosidl_service_type_support_t UpdateGeographicMap_service_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_UpdateGeographicMap_service_typesupport_map),
  rosidl_typesupport_c__get_service_typesupport_handle_function,
  &UpdateGeographicMap_Request_message_type_support_handle,
  &UpdateGeographicMap_Response_message_type_support_handle,
  &UpdateGeographicMap_Event_message_type_support_handle,
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_CREATE_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    geographic_msgs,
    srv,
    UpdateGeographicMap
  ),
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_DESTROY_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    geographic_msgs,
    srv,
    UpdateGeographicMap
  ),
  &geographic_msgs__srv__UpdateGeographicMap__get_type_hash,
  &geographic_msgs__srv__UpdateGeographicMap__get_type_description,
  &geographic_msgs__srv__UpdateGeographicMap__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace geographic_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_service_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_c, geographic_msgs, srv, UpdateGeographicMap)() {
  return &::geographic_msgs::srv::rosidl_typesupport_c::UpdateGeographicMap_service_type_support_handle;
}

#ifdef __cplusplus
}
#endif
